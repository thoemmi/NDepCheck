...
Exit codes:
   ...
   6    No dependency file found for an assembly in /d and /s 
        ...
        public int Run(string[] args) {
            ...
            for (; i < args.Length; i++) {
                foreach (var assemblyFilename in ExpandFilename(args[i])) {
                    string dependencyFilename = assemblyFilename + ".dep";
                    try {
                        DependencyRuleSet ruleSetForAssembly = DependencyRuleSet.Load(dependencyFilename, _directories, _verbose) ??
                                                    _defaultRuleSet;
        ...
        public static int Main(string[] args) {
            var main = new DependencyCheckerMain();
