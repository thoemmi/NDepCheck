    ...
    public class MainTests {
        ...
        public void GeneralSucceedingTest() {
            ...
            using (TextWriter tw = new StreamWriter(depFile, false, Encoding.Default)) {
                tw.Write(
                    @"
// Test dependencies for DependencyChecker
...
=:
    ...
    // DependencyChecker may use antlr and itself.
DependencyChecker ====> antlr

    // DependencyChecker must not use Windows Forms.
DependencyChecker.** ---! System.Windows.Forms.**

_TES  := asdasdasdasdasdasd
_TESTS := DependencyCheckerTest
...
_TESTS.dir1.* ---> _TESTS.dir1.dir3.*
    ...
    // SomeClass::AnotherMethod may use NamespacelessTestClassForDependencyChecker -
    // but this is questionable.
_TESTS.dir1.dir2.SomeClass::AnotherMethod ---? NamespacelessTestClassForDependencyChecker
...
// ------------------
    ...
    // current test class.
DependencyCheckerTest.UnitTests ---> **

    // All of system is ignored
% ()System.**

    // Classes in DependencyCheckerTests are shown separately, without the namespace
% DependencyCheckerTests.(**)

    // Classes in DependencyChecker are also shown separately, but with the namespace
% (DependencyChecker).*
...
% (*)
            ...
            using (TextWriter tw = new StreamWriter(outFile)) {
                ...
                Console.SetOut(tw);
                string[] args = { "/v", "/x", "/f=" + depFile, "DependencyCheckerTest.dll" };
                DependencyCheckerMain.Main(args);
        ...
        public void SmallGrapherTest() {
            ...
            var dg = new DependencyGrapher(new DependencyChecker.DependencyChecker()) { DOTFilename = "c:\\temp\\test.dot" };
        ...
        public void Exit1() {
            ...
            Assert.AreEqual(1, DependencyCheckerMain.Main(new[] { "/v", "-v", "-i", "/i=100", "-t", "/t", "-g=someDotFile.dot" }));
        }
        ...
        public void Exit3() {
            {
                ...
                Assert.AreEqual(3, DependencyCheckerMain.Main(new[] { "-x=" + depFile, "DependencyCheckerTestAssembly.dll" }));
                File.Delete(depFile);
            }
            {
                string depFile = Path.GetTempFileName();
