...
namespace NDepCheck.Rendering.TextWriting {
    public class PathWriter : IRenderer {
        ...
        public void Render(GlobalContext globalContext, IEnumerable<Dependency> dependencies, int? dependenciesCount, string argsAsString,
            string baseFileName, bool ignoreCase) {
            throw new NotImplementedException();
        }
    ...
    // TODO: Make PathMarker a Transformer
    public class PathMarker : IRenderer {
        ...
        private static void Write(bool backwards, AbstractPathMatch<Dependency, Item> countMatch,
            ...
            if (withHeader) {
                tw.WriteLine($"// Written {DateTime.Now} by {typeof(PathMarker).Name} in NDepCheck {Program.VERSION}");
            }
