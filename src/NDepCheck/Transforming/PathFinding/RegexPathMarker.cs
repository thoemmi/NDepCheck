...
namespace NDepCheck.Transforming.PathFinding {
    public class PathRegex : PathRegex<Item, Dependency, ItemMatch, DependencyMatch> {
        public PathRegex(string definition, Dictionary<string, ItemMatch> definedItemMatches, Dictionary<string, DependencyMatch> definedDependencyMatches, bool ignoreCase) : base(definition, definedItemMatches, definedDependencyMatches, ignoreCase) {
        }

        protected override ItemMatch CreateItemMatch(string pattern, bool ignoreCase) {
            return new ItemMatch(pattern, ignoreCase, anyWhereMatcherOk: false);
        }

        protected override DependencyMatch CreateDependencyMatch(string pattern, bool ignoreCase) {
            return DependencyMatch.Create(pattern, ignoreCase);
        }
    }

    public class RegexPathMarker : ITransformer {
        ...
        public void Configure([NotNull] GlobalContext globalContext, [CanBeNull] string configureOptions, bool forceReload) {
            ...
            throw new NotImplementedException("RegexPathMarker IS STILL A COPY-PASTE FROM PathMarker AND NOT YET FUNCTIONAL!!!!");
