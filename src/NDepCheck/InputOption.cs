...
//            var fileNames = new List<string>(Option.ExpandFilePatternToFullFileNames(_positive, ".dll", ".exe"));
//            List<string> negative = new List<string>(_negative.SelectMany(s => Option.ExpandFilePatternToFullFileNames(s, ".dll", ".exe")))
//                                    .ConvertAll(Path.GetFullPath);
//            fileNames.RemoveAll(f => negative.Contains(Path.GetFullPath(f)));
//            return fileNames.Select(fileName => _readerFactory.CreateReader(fileName, needsOnlyItemTails)).ToArray();
//        }
//    }
//}
